...
const first = (i) => i ? i[0] : fail(),
  rest = (i) => i ? i.slice(1, i.length) : "",
  unit = (v, i) => new Result(v, i),
  // Parsers: functions that take an input and return a
  // Result<match, remaining_input> or an error

  item = async (i) => unit(first(i), rest(i)),
  fail = async (i) => {
    throw "fail";
  },
  ...
  and = (p1, p2) =>
    (i) => p1(i).then((r) => p2(r.i).then((r2) => unit(r.v + r2.v, r2.i))),
  ...
  before = (p1, p2) =>
    (i) => p1(i).then((r) => p2(r.i).then((r2) => unit(r.v, r2.i))),
